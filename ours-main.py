...
def main(device, args):
    ...
    for iter in range(args.num_epochs):
        ...
        if iter % 1 == 0:
            ...
            if acc > best_acc:
                best_acc = acc
                best_model_state = model_glob.state_dict()
        ...
        for idx in idxs_users:
            ...
            for i, ((img, img_ema), label) in enumerate(train_loader_unlabeled):
                ...
                Lu = (F.cross_entropy(ema_model_out, targets_u, reduction='none') * mask).mean()

                # **KL Divergence Loss (Lkl)**
                sharpened_pseudo_label = pseudo_label1 ** (1 / args.T)
                sharpened_pseudo_label /= sharpened_pseudo_label.sum(dim=1, keepdim=True)
                log_softmax_pred = F.log_softmax(model_out, dim=-1)
                mask_low_confidence = (1 - mask).float()
                Lkl = (mask_low_confidence[:, None] * sharpened_pseudo_label *
                       (log_softmax_pred - torch.log(sharpened_pseudo_label + 1e-10))).sum(dim=-1).mean()

                # **Entropy Meaning Loss (Loss_em)**
                k_value = torch.topk(sharpened_pseudo_label, 2, dim=-1)[1]
                softmax_pred = torch.softmax(model_out, dim=-1)

                mask_k = torch.zeros_like(pseudo_label1).scatter_(1, k_value[:, :1], 1.0)
                label_indices = targets_u[:, None]
                mask_k.scatter_(1, label_indices, 1.0)

                yg = (softmax_pred * mask_k).sum(dim=-1, keepdim=True).clamp(1e-7, 1 - 1e-7)
                soft_ml = (1 - yg + 1e-7) / (k_value.size(1) - 1)
                mask_high_confidence = mask
                mask_pred = mask_high_confidence[:, None]
                mask_em = (1 - mask_k) * mask_pred
                mask_em = torch.where((mask_em == 1) & (softmax_pred > args.threshold_pl ** 2), torch.zeros_like(mask_em),
                                      mask_em)

                loss_em = -(
                        soft_ml * torch.log(softmax_pred + 1e-10) +
                        (1 - soft_ml) * torch.log(1 - softmax_pred + 1e-10)
                )
                loss_em = (loss_em * mask_em).sum() / (mask_em.sum() + 1e-10)

                # **Combine Losses**
                lambda_iccs = 0.01
                lambda_l = 0.01
                lambda_l2 = 10
                lambda_l1 = 0.0001 # 0.001, 0.01, 0.1
                loss = (
                        lambda_iccs * (Lu) + lambda_l * (Lkl + loss_em) +
                        lambda_l2 * dist_l2(model_local_t.state_dict(), model_glob.state_dict()) +
                        lambda_l1 * dist_l1(model_local_t.state_dict())
                )
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
