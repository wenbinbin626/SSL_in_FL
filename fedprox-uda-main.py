...
def dist(w0, w1):
    dist_w = 0.0
    for i in list(w0.keys())  :
                dist_w += (w0[i]-w1[i]).float().norm(2)**2
    dist_w = math.sqrt(dist_w)
    return dist_w
    ...
    for iter in range(args.num_epochs):
        ...
        for idx in idxs_users:
            ...
            for i, ((images1, images2), labels) in enumerate(train_loader_unlabeled):
                ...
                Lprox = 1 / 2 * dist(model_local.state_dict(), model_glob.state_dict())
